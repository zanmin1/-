			...
			<div class="layui-row layui-form layui-sch-g flex mt15">
				<div class="flex">
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block w160">
							<input type="text" placeholder="请输入名称" class="layui-input" id="name" name="name">
						</div>
					</div>
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block w160">
							<input type="text" placeholder="请输入名称" class="layui-input" id="themeName" name="themeName">
						</div>
					</div>
...
<script type="text/html" id="tbTollsBar">
	<div class="layui-col-xs12">
		<p class="table-title-ext1 fl"><i class="text-icon-ext1"></i>模块列表</p>
...
<script>
		...
		layui.use(['form', 'table', 'element', 'layer'], function () {
			...
			table.render({
				...
				cols: [[
					...
					, {field: 'name', title: '模块名称', align: 'center',event:'viewDetails', style:'color:#3d80f4; cursor:pointer;'}
					, {field: 'typeName', title: '模块类型', align: 'center'}
					, {field: 'regionName', title: '所属区域', align: 'center'}
					, {field: 'themeName', title: '主题名称', align: 'center'}
					, {field: 'useTypeName', title: '使用场景', align: 'center'}
			...
			table.on('tool(tableId)', function (obj) {
				...
				if (obj.event === 'viewDetails') {
					var index = layer.open({
						...
						area: [vWidth, vHeight],
						content: '${rc.getContextPath()}/dcl/dvModular/view.jhtml?uuid=' + obj.data.uuid
					});
				}
			});
	...
	// 添加
	function add() {
		var url = '${rc.getContextPath()}/dcl/dvModular/form.jhtml';
		var index = layer.open({
			skin: 'layer-ext-sch',
			type: 2,
			title: title+'-添加',
	...
	function edit() {
		if (selectItem == null) {
			...
			var url = '${rc.getContextPath()}/dcl/dvModular/form.jhtml?uuid=' + selectItem.uuid;
			var index = layer.open({
				skin: 'layer-ext-sch',
				type: 2,
				title: title +'-编辑',
	...
	function delConfirm() {
		$.ajax({
			type: 'POST',
			url: '${rc.getContextPath()}/dcl/dvModular/del.jhtml',
			data: {
