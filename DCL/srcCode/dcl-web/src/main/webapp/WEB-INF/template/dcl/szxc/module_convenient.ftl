<#--主题党日-->
<script>
${WECHAT_DOMAIN!''}/cct/handlingMatters/index.mhtml
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data && data.rows && data.rows.length > 0) {
                    var rows = data.rows;
                    showArticle('${infoOpenId}' == '' ? rows[0].hmId : '${infoOpenId}',1);
    ...
    function showArticle(hmId,type) {
        if(type==1){
            var catalogId = '${catalogId}';
            if(strInfo(catalogId)!='' && strInfo(catalogId)!='undefined' ){
                hmId = catalogId;
            }
        }
        console.log("===="+hmId);
