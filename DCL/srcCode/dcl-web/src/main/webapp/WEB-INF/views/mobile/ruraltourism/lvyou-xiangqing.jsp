        ...
        <div class="layui-row">
            ...
            <div class="layui-col-xs12 pl15 pr15 posi-rela z-index">
                <div class="layui-col-xs12 border-radius-4 box-shadow-ext1 bg-fff notice-margin-ext1 pl15 pt10 pr15 pb15">
                    <p class="font-size-16 cor-333">${bo.attractionsName}</p>
                    <div class="flex flex-ac flex-jb mt5">
                        <div class="flex flex-ac">
                            <p class="list-tag-ext4 font-size-12 cor-fff">${bo.attractionsTypeName}</p>
                            <p class="font-size-13 cor-green3 ml10">${bo.chargeTypeName}</p>
                        </div>
<%--                        <button class="layui-btn layui-btn-ext1 flex flex-ac"><em class="font-size-12 cor-666">实用信息</em><i class="layui-icon layui-icon-right font-size-16 cor-ccc ml5"></i></button>--%>
                ...
                <div class="layui-col-xs12 pb25" id="nr">
                    <p class="posi-rela text-lh1 font-size-15 cor-333 pl15 mt20"><i class="notic-bar-ext1"></i>景点介绍</p>
                    ${bo.attractionsDesc}
                </div>
            </div>
        </div>
...
<script>
        ...
        if(attlist!=null&&attlist.length>0){
            $.each(attlist,function (i,val){
                ...
                if(val.img=="img"){
                    ...
                    html+=  " <div class=\"swiper-slide img100\">\n" +
                         " <video src=\"${imgDomain}"+val.picUrl+"\"  style=\"width:100%;height:90%\"   controls=\"controls\"   autostart=\"false\" loop=\"true\" />\n" +
                        "  </div>";
                }
            })
        }else{
    ...
    $(function () {
        ...
        if('${bo.picUrl}'==null||'${bo.picUrl}'==''){
            ...
            $("#img").html('<img src="${uiDomain}/app-assets/_jzfp/fujian-xczx/images/notice/ui-notice-pic-21.png">')
        }else{
        ...
        $('.address').on('click',function(){
            ...
            if('${bo.x}'!=''){
                location.href ='<%=path%>/public/wap/article/showMap.jhtml?x=${bo.x}&y=${bo.y}';
            }else{
