	...
	*/
	...
	var checkWxJsApi = function checkWxJsApi(){
		...
		if (!isWeixinBrowser()) {//app初始化
			...
			try {
				...
				//}
				$.getScript(
					'https://res.wx.qq.com/open/js/jweixin-1.4.0.js',function(){
						layerIndex = layer.load();
						try{
							$.ajax({
								url: domain + '/wxjsapiticket/getJsSdkMsgFromGetJson.mhmtl',
								type: "get",
								data:{
									urlStr : getUrlStr(),
	...
	$.fn.extend({
		wapImgUpload : function(options) {
							...
							$cn.find(options.delBtnEle).show().attr('data-attaid',obj.attaId).bind('click',function(){
								...
								if (options.delConfirmFunc != 'undefined' && typeof(options.delConfirmFunc) == 'function') {
									new options.delConfirmFunc(function(closeConfirmWinCallBack){
										...
										fileDel(_this, eleId, options.isDelFromDb, options.delCallBackFunc, closeConfirmWinCallBack);
									});
								} else {//默认的弹窗方式
									ffcswxjsapi_popupNewWindow('确定删除图片?',{
										...
										fileDel(_this, eleId, options.isDelFromDb, options.delCallBackFunc);
									});
								}
							});
						}
			...
			*/
			function fileDel(node, eleId, isDelFromDb, delCallBackFunc, closeConfirmWinCallBack) {
