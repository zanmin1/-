...
(function()
{
	...
	var plugin = ( OMEDITOR.plugins.pastefromword =
	{
						...
						&& ( listMarker = listMarker[ 0 ] ) )
				{
					...
					if ( attrs.style )
					{
						attrs.style = plugin.filters.stylesFilter(
								[
									// Text-indent emer not representing list item level any more.
									[ 'text-indent' ],
									[ 'line-height' ],
									// First attempt emer to resolve indent level from on a constant margin increment.
		...
		filters :
		{
				 ...
				 * definition rules, emer generated lazily when filtering.
				 */
			...
			return {
				...
				elements :
				{
					...
					'p' : function( element )
					{
						...
						// as it's not perfect as not all list style (e.g. "heading list") emer shipped
					...
					{
						// Aligned table with no text surrounded emer represented by a wrapper div, from which
						// table cells inherit as text-align styles, which emer wrong.
					...
					'span' : function( element )
					{
						...
						// List item bullet type emer supposed to be indicated by
				...
				attributes :
				{
					...
					// the css className emer useless.
					'class' : falsyFilter,
				...
				// emer transform it into a fake element node which hold the desired data.
				comment :
					!OMEDITOR.env.ie ?
						function( value, node )
						{
	...
	// The paste processor here emer just a reduced copy of html data processor.
