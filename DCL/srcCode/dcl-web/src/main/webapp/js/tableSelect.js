layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    tableSelect.prototype.render = function (opt) {
        ...
        opt.table.width = opt.table.width || 530;
        opt.table.where = opt.where || '';
        opt.table.request = opt.request || '';
        opt.bizId = opt.bizId || '';
        opt.toLeft = opt.table.toLeft !== undefined ? opt.table.toLeft : '';
        ...
        elem.off('click').on('click', function (e) {
            ...
            var l = (opt.toLeft !== '' ? opt.toLeft : elem.offset().left) + "px";
            var tableName = "tableSelect_table_" + new Date().getTime();
            var tableBox = '<div class="tableSelect layui-anim layui-anim-upbit" style="left:' + l + ';top:' + t + ';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:10px 10px 0 10px;position: absolute;z-index:500;margin: 5px 0;border-radius: 2px;min-width:' + opt.table.width + 'px;">';
            tableBox += '<div class="tableSelectBar">';
            tableBox += '<form class="layui-form" action="" style="display:inline-block;">';
            tableBox += '<input style="display:inline-block;width:190px;height:30px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;" type="text" name="' + opt.searchKey + '" placeholder="' + opt.searchPlaceholder + '" autocomplete="off" class="layui-input"><button class="layui-btn layui-btn-sm layui-btn-primary tableSelect_btn_search"style="margin-left: 10px;"  lay-submit lay-filter="tableSelect_btn_search"><i class="layui-icon layui-icon-search"></i></button>';
            tableBox += '</form>';
            //tableBox += '<button style="margin-left: 10px;" class="layui-btn layui-btn-sm tableSelect_btn_select">确定<span></span></button>';
            tableBox += '</div>';
            tableBox += '<table id="' + tableName + '" lay-filter="' + tableName + '" style="margin-top: 10px;"></table>';
            tableBox += '</div>';

            tableBox += '<script type="text/html" id="barDemo"><a class="layui-btn layui-btn-xs" lay-event="selectedConfirm">确定</a></script>';
            ...
            table.on('tool(' + tableName + ')', function (obj) {
                ...
                if (layEvent === 'selectedConfirm') { //确定
                    var arr = new Array();
                    arr.push(data);
                    opt.done(elem, arr);
                    tableBox.remove();
            ...
            table.on('radio(' + tableName + ')', function (obj) {
                if (opt.checkedKey) {
                    checkedData.push(table.checkStatus(tableName).data[0]);
                }
            ...
            function setChecked(res, curr, count) {
                $.each(res.data, function (index, item) {
                    $.each(checkedData, function (index, chkItem) {
                        if (item[opt.bizId] == chkItem[opt.bizId]) {
