...
(function ($) {
	$.hunterTimePicker = function (id,box, options) {
		var dates = {
		...
		function hourEvent(){
			time_wrap.on('click', '.Hunter-hour', function(event){
				...
				if(temp.length > 1){
					...
					$("#"+emId+"_text").val(hourValue+":"+temp[1]);
				}else{
					box.val(hourValue+":00");
					$("#"+emId+"_text").val(hourValue+":00");
				}
	...
	$.fn.extend({
		hunterTimePicker: function (id,options) {
			options = $.extend({}, options);
			this.each(function () {
				new $.hunterTimePicker(id,this, options);
			});
			return this;
		}
	});
