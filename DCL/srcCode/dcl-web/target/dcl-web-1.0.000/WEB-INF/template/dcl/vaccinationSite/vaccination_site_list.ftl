	...
	<div id="jqueryToolbar">
		<div class="ConSearch">
			...
			<div class="fl">
				<ul>
					<li>接种点名称：</li>
					<li><input class="inp1" type="text" id="vaccinationsiteName" name="vaccinationsiteName" /></li>
				</ul>
			</div>
	        <div class="btns">
				<ul>
...
<script type="text/javascript">
	...
	function loadList() {
		$('#list').datagrid({
			...
			columns: [[
				{field:'vaccinationsiteName', title:'疫苗接种点名称', align:'center', width:100},
				{field:'regionName', title:'所属区域', align:'center', width:100},
				// {field:'vaccinationTime', title:'疫苗接种时间', align:'center', width:100},
	...
	//新增
	function add() {
		var url = '${rc.getContextPath()}/dcl/vaccinationSite/form.jhtml';
		showMaxJqueryWindow('新增', url, 500, 250);
	}
	...
	function edit() {
		...
		if (rows.length != 1) {
			...
			var url = '${rc.getContextPath()}/dcl/vaccinationSite/form.jhtml?id=' + rows[0].uuid;
			showMaxJqueryWindow('编辑', url, 500, 250);
		}
	}
	
	//详情
	function detail(id) {
		var url = "${rc.getContextPath()}/dcl/vaccinationSite/view.jhtml?id=" + id;
		showMaxJqueryWindow('详情', url, 500, 250);
	}
		...
		if (rows.length != 1) {
			...
			$.messager.confirm('提示', '您确定删除选中的信息吗?', function(r) {
				if (r) {
					...
					$.ajax({
						type: 'POST',
						url: '${rc.getContextPath()}/dcl/vaccinationSite/del.json',
						data: {
							uuid: rows[0].uuid
						},
						dataType: 'json',
