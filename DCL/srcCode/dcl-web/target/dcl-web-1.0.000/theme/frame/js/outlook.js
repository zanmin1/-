    ...
    for (i = 0; i < outlookbar.titlelist.length; i++) {
        if (outlookbar.titlelist[i].isdefault == 1 && outlookbar.titlelist[i].sortname == sortname) {
            ...
            for (j = 0; j < outlookbar.itemlist[i].length; j++) {
                output += "<li id="
                        + outlookbar.itemlist[i][j].sortname + j
                        + " onclick=\"changeframe('"
            ...
            output+= "</ul></div>"
        } else if (outlookbar.titlelist[i].isdefault == 0 && outlookbar.titlelist[i].sortname == sortname) {
            output += "<div class=list_tilte_onclick id=sub_sort_" + i + " onclick=\"hideorshow('sub_detail_" + i + "')\">";
            output += "<span>" + outlookbar.titlelist[i].title + "</span>";
            output += "</div>";
            output += "<div class=list_detail id=sub_detail_" + i + " style='display:none;'><ul>";
            for (j = 0; j < outlookbar.itemlist[i].length; j++) {
                output += "<li id="
                        + outlookbar.itemlist[i][j].sortname + j
                        + " onclick=\"changeframe('"
...
function getbyitem(item) {
    ...
    for (i = 0; i < outlookbar.titlelist.length; i++) {
        if (outlookbar.titlelist[i].title == item) {
            ...
            output += "<span>" + outlookbar.titlelist[i].title
                    + "</span>";
            output += "</div>";
            output
            ...
            for (j = 0; j < outlookbar.itemlist[i].length; j++) {
                output += "<li id=" + outlookbar.itemlist[i][j].sortname + "_" + j
                        + " onclick=\"changeframe('" + outlookbar.itemlist[i][j].title
                        + "','" + outlookbar.titlelist[i].title + "','"
...
function changeframe(item, sortname, src) {
    if (item != "" && sortname != "") {
        window.top.frames['topFrame'].showPosition(sortname, item);
    }
    ...
    if (src != "") {
        ...
        window.top.frames['mainFrame'].location = window.top.frames['topFrame'].getObject("loadimg").src;
        //window.top.frames['topFrame'].getObject("tip").innerHTML = "页面加载中...";
        //window.top.frames['topFrame'].getObject("tip").style.display = "";
...
function initinav(sortname) {
    ...
    //window.top.frames['mainFrame'].location="mms/tree.gsp"
}