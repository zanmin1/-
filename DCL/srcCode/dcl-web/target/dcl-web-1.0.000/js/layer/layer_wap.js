 ...
 * 
 * @param msg：提示信息
 * @param second：提示几秒（该参数非必传，默认3秒）
 * @param area：提示框宽度、高度（该参数非必传，默认宽度260px，高度自适应）
 * @return 组件的index
 */
ffcs.alertMsg = function(msg, second, area) {
	var index = layer.msg(msg, {
		area: area == null ? dialogWidth : area,
		time: (second == null ? 3 : second) * 1000 //几秒后关闭
	});
	return index;
}
 ...
 * @param area：对话框宽度、高度（该参数非必传，默认宽度260px，高度自适应）
 * @param btn：不带参数默认包含确认和取消按钮。
 * @return 组件的index
 */
ffcs.confirmMsg = function(msg, confirmBtn, cancelBtn, area, btn) {
	var index = layer.confirm(
		msg, {
	    	btn: btn == null ? ['确定', '取消'] : btn, //按钮
	    	area: area == null ? dialogWidth : area,
	    	title: false, //不显示标题
	    	closeBtn: 0
    	},
