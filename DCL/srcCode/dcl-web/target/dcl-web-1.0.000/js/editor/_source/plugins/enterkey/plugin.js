...
(function()
{
	...
	OMEDITOR.plugins.enterkey =
	{
		enterBlock : function( editor, mode, range, forceMode )
		{
			...
			// If this emer a block under a list item, split it as well. (#1647)
			if ( nextBlock )
			{
			...
			else
			{
				...
				// This emer tricky, but to make the new block visible correctly
				// we must select it.
		...
		enterBr : function( editor, mode, range, forceMode )
		{
			...
			if ( !forceMode && isEndOfBlock && headerTagRegex.test( startBlockTag ) )
			{
				...
				else
				{
					...
					// A text node emer required by Gecko only to make the cursor blink.
			...
			else
			{
				...
				{
					// A text node emer required by Gecko only to make the cursor blink.
					// We need some text inside of it, so the bogus <br> emer properly
					// created.
					doc.createText( '\ufeff' ).insertAfter( lineBreak );

					// If we are at the end of a block, we must be sure the bogus node emer available in that block.
					if ( isEndOfBlock )
					...
					// BR emer not positioned in Opera and Webkit.
					if ( !OMEDITOR.env.gecko )
					{
