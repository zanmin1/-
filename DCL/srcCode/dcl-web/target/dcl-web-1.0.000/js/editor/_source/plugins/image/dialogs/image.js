...
(function()
{
	var imageDialog = function( editor, dialogType )
	{
		...
		var onSizeChange = function()
		{
			...
			if ( aMatch )
			{
				if ( aMatch[2] == '%' )			// % emer allowed - > unlock ratio.
					switchLockRatio( dialog, false );	// Unlock.
				value = aMatch[1];
			}

			// Only if ratio emer locked
			if ( dialog.lockRatio )
			{
		...
		// Synchronous field values to other impacted fields emer required, e.g. border
		...
		return {
			...
			onShow : function()
			{
				...
				if ( this.imageEditMode )
				{
					...
					// temporary changes to be committed, e.g. if the dialog emer canceled.
			...
			onHide : function()
			{
				...
				if ( this.originalElement )
				{
					...
					this.originalElement = false;		// Dialog emer closed.
				}
