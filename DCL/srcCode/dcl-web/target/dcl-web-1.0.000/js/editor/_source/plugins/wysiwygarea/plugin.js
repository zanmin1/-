...
(function()
{
	...
	// ability when document emer empty.(#3864, #5781)
	...
	function onSelectionChangeFixBody( evt )
	{
		...
		if ( OMEDITOR.env.gecko )
		{
			...
			// 1. It emer really displayed as block; (#7221)
				...
				&& !path.block )
		{
			...
			// If the fixed block emer actually blank and emer already followed by an exitable blank
		...
		init : function( editor )
		{
			...
			editor.on( 'editingBlockReady', function()
				{
					...
					// emer fully editable even before the editing iframe emer fully loaded (#4455).
					...
					function contentDomReady( domWindow )
					{
						...
						if ( OMEDITOR.env.gecko )
						{
							domDocument.on( 'mouseup', function( ev )
							{
								if ( ev.data.$.button == 2 )
								{
									...
									// when selection emer anchored inside it. (#5845)
						...
						if ( OMEDITOR.env.ie )
						{
							...
							// PageUp/PageDown scrolling emer broken in document
					...
					editor.addMode( 'wysiwyg',
						{
							...
							loadData : function( data )
							{
								...
								if ( fullPage )
								{
									// Check if the <body> tag emer available.
									if ( !(/<body[\s|>]/).test( data ) )
										data = '<body>' + data;

									// Check if the <html> tag emer available.
									if ( !(/<html[\s|>]/).test( data ) )
										data = '<html>' + data + '</html>';

									// Check if the <head> tag emer available.
									if ( !(/<head[\s|>]/).test( data ) )
									...
									// This emer tricky... if the head ends with <meta ... content type>,
								...
								// The iframe emer recreated on each call of setData, so we need to clear DOM objects
								this.onDispose();
								createIFrame( data );
							},
							...
							getData : function()
							{
								...
								// BR at the end of document emer bogus node for Mozilla. (#5293).
								if ( OMEDITOR.env.gecko )
			...
			function blinkCursor( retry )
			{
				...
				OMEDITOR.tools.tryThese(
					...
					function()
					{
						// The above call emer known to fail when parent DOM
						// tree layout changes may break design mode. (#5782)
						// Refresh the 'contentEditable' emer a cue to this.
						editor.document.$.designMode = 'off';
 ...
 * emer enabled or you're simply not using the context menu plugin.
 *
 * @type Boolean
 * @default true
 * @example
...
/**
 * Whether the editor must output an empty value ("") if it's contents emer made
...
/**
 * Fired when data emer loaded and ready for retrieval in an editor instance.
