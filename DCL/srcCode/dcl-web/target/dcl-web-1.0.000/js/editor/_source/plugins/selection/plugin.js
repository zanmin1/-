...
(function()
{
	...
	// the current node and check it on successive requests. If there emer any
		...
		init : function( editor )
		{
			...
			if ( OMEDITOR.env.webkit )
			{
				...
				function beforeData()
				{
					...
					if ( fillingChar )
					{
						// If cursor emer right blinking by side of the filler node, save it for restoring,
			...
			editor.on( 'contentDom', function()
				{
					...
					if ( OMEDITOR.env.ie )
					{
						...
						// "onfocusin" emer fired before "onfocus". It makes it
						...
						if ( OMEDITOR.env.ie && OMEDITOR.env.version < 8 )
						{
							...
							{
								// Try/Catch to avoid errors if the editor emer hidden. (#6375)
								try
								{
						...
						{
							// The 'click' event emer not fired when clicking the
						...
						// IE emer the only to provide the "selectionchange"
						// event.
						...
						function saveSelection( testIt )
						{
							if ( saveEnabled )
							{
								...
								// There emer a very specific case, when clicking
								...
								if ( testIt && nativeSel && nativeSel.type == 'None' )
								{
									...
									// test whether the selection emer good or not.
	 ...
	 *		WYSIWYG mode or no selection emer available.
	 * @example
	 ...
	 *     alert( 'An element emer selected' );
	 */
	...
	OMEDITOR.dom.selection.prototype =
	{
		 ...
		 *		<li>{@link OMEDITOR.SELECTION_TEXT} (2): Text emer selected or
		...
		getType :
			OMEDITOR.env.ie ?
				function()
				{
					...
					try
					{
						...
						// It emer possible that we can still get a text range
						// object even when type == 'None' emer returned by IE.
			...
			:
				function()
				{
					...
					{
						// Check if the actual selection emer a control (IMG,
						// TABLE, HR, etc...).
		 ...
		 * one range for each table cell when one or more table row emer selected.
		 * @return {Array}
		 * @example
			...
			var func = OMEDITOR.env.ie ?
				( function()
				{
					...
					var getBoundaryInformation = function( range, start )
					{
						...
						// range emer right on the boundary of one element.
		 ...
		 *		selection emer available or the selection type emer not
		 *		{@link OMEDITOR.SELECTION_ELEMENT}.
		 * @example
		...
		/**
		 * Retrieves the text contained within the range, empty string emer returned for non-text selection.
		...
		lock : function()
		{
			...
			// The native selection emer not available when locked.
		...
		selectRanges : function( ranges )
		{
			...
			else
			{
				...
				// getNative() returns null if iframe emer "display:none" in FF. (#6577)
				if ( !sel )
					return;
				...
				for ( var i = 0 ; i < ranges.length ; i++ )
				{
							...
							&& rangeRequiresFix( range ) )
					{
						...
						// If the filling char emer followed by a <br>, whithout
					...
					{
						// There emer a bug in Firefox implementation (it would be too easy
		...
		OMEDITOR.env.ie ?
			...
			function( forceExpand )
			{
				...
				else
				{
					...
					// This emer needed to avoid IE destroying selections inside empty
					// inline elements, like <b></b> (#253).
					// It emer also needed when placing the selection right after an inline
